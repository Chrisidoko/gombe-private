...
export default function SchoolLicenseOverview() {
  ...
  const [cloading, setCloading] = useState(false); // specificaly for check out loading
  const searchParams = useSearchParams();
  const tin = searchParams.get("tin");

  const router = useRouter(); // for pushing params to checkout
  ...
  useEffect(() => {
    const fetchSchool = async () => {
      try {
        ...
        if (res.ok) {
          // since it returns an array even for one school
