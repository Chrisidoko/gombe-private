...
const ColumnFiltersLabel = ({
    ...
    return (
      <span className={cx("truncate", className)}>
        {columnFilterLabels.map((value, index) => (
          <span key={value} className={cx("font-semibold text-indigo-600")}>
            {value}
  ...
  return (
    <>
      <span className={cx("font-semibold text-indigo-600", className)}>
  ...
  const getDisplayedFilter = () => {
    switch (type) {
      ...
      case "number":
        ...
        const defaultConditionFilter = { condition: "", value: ["", ""] };
        const conditionFilter =
          (selectedValues as ConditionFilter) || defaultConditionFilter;
        const isBetween = conditionFilter.condition === "is-between";
        return (
          <div className="space-y-2">
            <Select
              value={conditionFilter.condition}
              onValueChange={(value: string) => {
                setSelectedValues({
                  condition: value,
                  value: [
                    value !== "" ? conditionFilter?.value?.[0] || "" : "",
                    "",
                  ],
                });
              }}
            ...
            <div className="flex w-full items-center gap-2">
              ...
              />
              <Input
                disabled={!conditionFilter.condition}
                type="number"
                placeholder="N0"
                className="sm:[&>input]:py-1"
                // Always provide a defined string value
                value={conditionFilter.value[0] || ""}
                onChange={(e) => {
                  setSelectedValues({
                    ...
                    value: [
                      e.target.value,
                      isBetween ? conditionFilter.value[1] || "" : "",
                    ],
                  });
                }}
              />
              {conditionFilter.condition === "is-between" && (
                <>
                  ...
                  <Input
                    ...
                    className="sm:[&>input]:py-1"
                    // Always provide a defined string value
                    value={conditionFilter.value[1] || ""}
                    onChange={(e) => {
                      setSelectedValues({
                        condition: conditionFilter.condition,
                        value: [conditionFilter.value[0] || "", e.target.value],
                      });
                    }}
                  />
                </>
    ...
    <Popover>
      <PopoverTrigger asChild>
        <button
          type="button"
          className={cx(
            "flex w-full items-center gap-x-1.5 whitespace-nowrap rounded-md border border-gray-300 px-2 py-1.5 font-medium text-gray-600 hover:bg-gray-50 sm:w-fit sm:text-xs",
            selectedValues &&
