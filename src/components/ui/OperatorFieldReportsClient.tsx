  ...
  async function handleAction(id: string, action: "acknowledge" | "close") {
    ...
    try {
      ...
      setReports((prev) => prev.map((r) =>
        r.id === id
          ? { ...r, status: updated.status, acknowledged_at: updated.acknowledged_at ?? r.acknowledged_at, closed_at: updated.closed_at ?? r.closed_at }
          : r
      ));
    } catch {
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">
          Inspector findings submitted from the field.
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {(["all", "open", "acknowledged", "closed"] as StatusFilter[]).map((f) => (
          ...
          >
            ...
            <span className={`text-xs font-bold px-1.5 py-0.5 rounded-full ${filter === f ? "bg-white/20 text-white" : "bg-gray-100 text-gray-500"}`}>
              {counts[f]}
            </span>
          </button>
        ))}
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 bg-white rounded-2xl border border-gray-200">
          ...
          <p className="text-xs text-gray-400 mt-1">
            {filter === "all" ? "No field reports have been submitted yet." : `No ${filter} reports at this time.`}
          </p>
        </div>
      ) : (
        <ul className="space-y-3">
          {filtered.map((r) => {
            ...
            return (
              <li key={r.id} className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                <button
                  type="button"
                ...
                >
                  <div className="flex items-center justify-center w-9 h-9 rounded-xl bg-gray-50 border border-gray-100 shrink-0 mt-0.5">
                    ...
                    {r.status === "acknowledged"  && <CheckCircle2 className="w-4 h-4 text-blue-500" />}
                    {r.status === "closed"        && <XCircle      className="w-4 h-4 text-gray-400" />}
                  </div>
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-3 mt-1 text-xs text-gray-400 flex-wrap">
                      <span className="flex items-center gap-1"><Building2 className="w-3 h-3" />{r.school_name}</span>
                      <span className="flex items-center gap-1"><User className="w-3 h-3" />{r.inspector_name}</span>
                      <span className="flex items-center gap-1"><Calendar className="w-3 h-3" />{formatDate(r.created_at)}</span>
                    </div>
                  </div>
                ...
                {isExpanded && (
                  <div className="border-t border-gray-100 px-5 py-5 space-y-5">
                    <div className="flex items-center gap-3 bg-gray-50 rounded-xl px-4 py-3">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-gray-800">{r.inspector_name}</p>
                        <p className="text-xs text-gray-400">{r.inspector_email}</p>
                      </div>
                    </div>

                    <div>
                      <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Findings</p>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs text-gray-500">
                      {[
                        ...
                        { label: "Acknowledged", value: formatDate(r.acknowledged_at) },
                        { label: "Closed",       value: formatDate(r.closed_at) },
                      ].map((t) => (
                        <div key={t.label}>
                          <p className="font-bold uppercase tracking-wider text-gray-400 mb-0.5">{t.label}</p>
                          <p>{t.value}</p>
                        </div>
                      ))}
                    </div>

                    {actionError[r.id] && <p className="text-xs text-red-500 font-medium">{actionError[r.id]}</p>}
                      ...
                      <div className="flex items-center gap-3 pt-1">
                        {r.status === "open" && (
                          <button
                            ...
                            className="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-blue-700 bg-blue-50 border border-blue-200 rounded-xl hover:bg-blue-100 disabled:opacity-50 transition"
                          >
                            {actionLoading === r.id + "acknowledge" ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <CheckCircle2 className="w-3.5 h-3.5" />}
                            Acknowledge
                          </button>
                        )}
                        <button
                          ...
                          className="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-gray-600 bg-gray-100 border border-gray-200 rounded-xl hover:bg-gray-200 disabled:opacity-50 transition"
                        >
                          {actionLoading === r.id + "close" ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <XCircle className="w-3.5 h-3.5" />}
                          Close Report
                        </button>
                      </div>
                    )}
                    ...
                    {r.status === "closed" && (
                      <p className="text-xs text-gray-400 italic">
                        This report is closed and will be permanently deleted 30 days after closing.
                      </p>
                    )}
                  </div>
                )}
